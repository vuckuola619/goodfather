...
def calculate_dimensions(answers: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        scores["connection"] = (
            round(sum(connection_vals) / len(connection_vals), 2)
            if connection_vals
            else 3.0
        )
        ...
        scores["selfRegulation"] = (
            round(sum(self_reg_vals) / len(self_reg_vals), 2) if self_reg_vals else 3.0
        )
        ...
        scores["faithAdab"] = (
            round(sum(faith_vals) / len(faith_vals), 2) if faith_vals else 3.0
        )
        ...
        if c4_val == "belum":
            c4_score = 5.0
        elif c4_val == "pernah didampingi":
            c4_score = 4.0
        elif c4_val == "tidak tahu":
            c4_score = 3.0
        elif c4_val == "pernah sendiri":
            c4_score = 2.0
...
def determine_result_type(
    scores: Dict[str, float], age_group: str, concerns: List[str]
) -> str:
    """
    ...
    if age_group in ["10–12 tahun", "13–17 tahun"] and any(
        item in concerns for item in ["sering melawan"]
    ):
...
def post_score() -> Response:
    ...
    try:
        ...
        return jsonify({"success": True, "scores": scores, "resultType": result_type})
    except Exception as e:
...
def post_backup() -> Response:
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Failed to save backup: {str(e)}"}
        ), 500
...
def health_check() -> Response:
    ...
    return jsonify({"status": "healthy", "timestamp": time.time(), "app": "GoodFather"})
